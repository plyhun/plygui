...
impl ::std::ops::BitOr<BoundarySizeParam> for BoundarySizeParam {
	...
	fn bitor(self, rhs: BoundarySizeParam) -> Self::Output {
		...
		let mut bottom = 0;
		match self {
			BoundarySizeParam::OrthoHorizontal(lr) => {left = lr; right = lr;},
			BoundarySizeParam::OrthoVertical(tb) => {top = tb; bottom = tb;},
			BoundarySizeParam::Left(l) => {left = l},
			BoundarySizeParam::Top(t) => {top = t;},
			BoundarySizeParam::Right(r) => {right = r;},
			BoundarySizeParam::Bottom(b) => {bottom = b;}
		}
		match rhs {
			BoundarySizeParam::OrthoHorizontal(lr) => {left = lr; right = lr;},
			BoundarySizeParam::OrthoVertical(tb) => {top = tb; bottom = tb;},
