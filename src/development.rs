...
pub struct SingleContainerBase {
    pub child: Option<Box<dyn controls::Control>>,
}

impl Default for SingleContainerBase {
    fn default() -> Self {
        SingleContainerBase { child: None }
    }
}

pub struct SingleContainer<T: SingleContainerInner> {
    base: SingleContainerBase,
    inner: T,
}

impl<T: SingleContainerInner> HasBase for SingleContainer<T> {
    type Base = SingleContainerBase;
...
impl<T: SingleContainerInner> HasInner for SingleContainer<T> {
    ...
    #[inline]
    fn with_inner(inner: Self::Inner, _: Self::Params) -> Self {
        SingleContainer { inner: inner, base: SingleContainerBase::default() }
    }
    #[inline]
    fn as_inner(&self) -> &Self::Inner {
        &self.inner
...
pub struct MultiContainerBase {
    pub children: Vec<Box<dyn controls::Control>>
}
impl Default for MultiContainerBase {
    fn default() -> Self {
        MultiContainerBase { children: Vec::with_capacity(2) }
    }
}

pub struct MultiContainer<T: MultiContainerInner> {
    base: MultiContainerBase,
    inner: T,
}

impl<T: MultiContainerInner> HasBase for MultiContainer<T> {
    type Base = MultiContainerBase;
...
impl<T: MultiContainerInner> HasInner for MultiContainer<T> {
    ...
    #[inline]
    fn with_inner(inner: Self::Inner, _: Self::Params) -> Self {
        MultiContainer { inner: inner, base: MultiContainerBase::default() }
    }
    #[inline]
    fn as_inner(&self) -> &Self::Inner {
        &self.inner
