...
impl ToTokens for Define {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let ident_inner = Ident::new(&format!("{}Inner", ident).to_upper_camel_case(), Span::call_site());
        	...
        	if let Some(ref custom) = self.custom {
        	    for block in custom.blocks.iter() {
	        		match block.name.to_string().as_str() {
	        			...
	        			"constructor" => {
		        			let custom = &block.custom;
		        			let new_ident = Ident::new(&format!("New{}", ident).to_upper_camel_case(), Span::call_site());
        ...
        let abstract_impl = if self._abstract {
            ...
            let constructor_inner = Ident::new(&format!("New{}Inner", ident).to_upper_camel_case(), Span::call_site());
            quote!{
